...
class Progetto
{
    ...
    function getElencoProgetti($filtro = null)
    {
        global $db;
        $temp = '';
        if ($filtro != null) {
        ...
        return $db->get_results("SELECT p.*,c.nome as cliente_nome FROM progetti p,clienti c WHERE p.codice_cliente=c.codice_cliente $temp ORDER BY nome");
    }
